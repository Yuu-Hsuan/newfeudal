...
class Feudal:
    ...
    def __init__(self, sess, ob_space, nbatch, nsteps, d, k, c, reuse=False, data_format='NCHW'):
        ...
        def non_spatial_output(x, channels, name, temperature=1.5):
            logits = fully_connected(x, channels, activation_fn=None, scope="non_spatial_output/flat/{}".format(name))
            # 對 logits 進行 clipping，防止數值過大或過小
            clipped_logits = tf.clip_by_value(logits, -10.0, 10.0)
            # 使用溫度參數平滑分佈
            smoothed_logits = clipped_logits / temperature
            return tf.nn.softmax(smoothed_logits)

        def spatial_output(x, name, temperature=1.5):
        ...
        # 在這裡進行標準化 - 直接計算標準差
        SCREEN = (SCREEN - tf.reduce_mean(SCREEN)) / (tf.sqrt(tf.reduce_mean(tf.square(SCREEN - tf.reduce_mean(SCREEN)))) + epsilon)
        MINIMAP = (MINIMAP - tf.reduce_mean(MINIMAP)) / (tf.sqrt(tf.reduce_mean(tf.square(MINIMAP - tf.reduce_mean(MINIMAP)))) + epsilon)
        FLAT = (FLAT - tf.reduce_mean(FLAT)) / (tf.sqrt(tf.reduce_mean(tf.square(FLAT - tf.reduce_mean(FLAT)))) + epsilon)
        ...
        with tf.variable_scope('model', reuse=reuse):
            ...
            with tf.variable_scope('manager'):
                ...
                '''
                g_ = fully_connected(manager_LSTM_input, d, activation_fn=tf.nn.relu, scope="manager_goal_fc")
                g_ = tf.reshape(g_, (nenvs, nsteps, d))
                print("g_", g_.shape)
                '''
            ...
            with tf.variable_scope('worker'):
                ...
                '''
                # 初始化狀態（隱藏狀態和細胞狀態）
                # 假設我們直接通過全連接層來生成隱藏狀態和細胞狀態
                # 如果之前有一個 LSTMStateTuple，這裡也會生成類似的結構
                # convLSTM_state = tf.nn.rnn_cell.LSTMStateTuple(
                #     c=tf.zeros([nenvs, nsteps, res, res, filters], dtype=tf.float32),  # 細胞狀態
                #     h=tf.zeros([nenvs, nsteps, res, res, filters], dtype=tf.float32)   # 隱藏狀態
                # )

                # 上面的 `convLSTM_outputs` 和 `convLSTM_state` 可以替代原來的 RNN 輸出
                '''
                ...
                '''
                ## 這裡的 convLSTM_inputs 可以替代原來的 RNN 輸入
                # 假設 convLSTM_inputs 是原來的輸入
                fc_out = fully_connected(convLSTM_inputs, filters, activation_fn=tf.nn.relu, scope='worker_fc')

                # 保證輸出的形狀和原來的一樣
                convLSTM_outputs = tf.reshape(fc_out, [nenvs, nsteps, res, res, filters])
                
                print("convLSTM_outputs shape: ", convLSTM_outputs.shape)
